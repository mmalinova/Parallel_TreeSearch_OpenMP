...
#pragma omp parallel sections num_threads(threads_num) 
		{
			// Creating section to traverse the left subtree
#pragma omp section
			{
			...
			// Creating section to traverse the right subtree
#pragma omp section
			{
				//thread_id = omp_get_thread_num();
				//printf("Thread = %d execute of %d threads.\n", thread_id, threads_num);
...
#pragma omp master    // only main thread execude this block of code
	if (!tree)
		return NULL;  // if the value is not found return NULL
	else
		return tree;  // if the value is found return that tree
